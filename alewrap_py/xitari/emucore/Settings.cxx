...
Settings::Settings(OSystem* osystem) : myOSystem(osystem) {
    ...
    setInternal("ssdir", std::string(".") + BSPF_PATH_SEPARATOR);
    ...
    setInternal("working_dir",  std::string(".") + BSPF_PATH_SEPARATOR);
...
void Settings::loadConfig(const char* config_file){
    ...
    std::string::size_type equalPos, garbage;

    std::ifstream in(config_file);
    if(!in || !in.is_open()) {
    	//std::cerr << "Warning: couldn't load settings file: " << config_file << "\n";
        return;
    }
    ...
    while(getline(in, line)) {
        ...
        if((equalPos = line.find("=")) == std::string::npos)
          continue;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
std::string Settings::loadCommandLine(int argc, char** argv)
{
...
void Settings::usage() {
    ...
    std::cerr << "\n" 
       ...
       "\n"
    ; // Closing the std::std::cerr statement
}
...
void Settings::saveConfig()
{
  ...
  std::ofstream out(myOSystem->configFile().c_str());
  if(!out || !out.is_open())
  {
    std::cerr << "Error: Couldn't save settings file\n";
    return;
  }

  out << ";  Stella configuration file" << std::endl
      << ";" << std::endl
      << ";  Lines starting with ';' are comments and are ignored." << std::endl
      << ";  Spaces and tabs are ignored." << std::endl
      << ";" << std::endl
      << ";  Format MUST be as follows:" << std::endl
      << ";    command = value" << std::endl
      << ";" << std::endl
      << ";  Commmands are the same as those specified on the commandline," << std::endl
      << ";  without the '-' character." << std::endl
      << ";" << std::endl
      << ";  Values are the same as those allowed on the commandline." << std::endl
      << ";  Boolean values are specified as 1 (or true) and 0 (or false)" << std::endl
      << ";" << std::endl;
  ...
  for(unsigned int i = 0; i < myInternalSettings.size(); ++i)
  {
    out << myInternalSettings[i].key << " = " <<
           myInternalSettings[i].value << std::endl;
  }
...
int Settings::getInt(const std::string& key, bool strict) const {
    ...
    if((idx = getInternalPos(key)) != -1) {
        ...
        if((idx = getExternalPos(key)) != -1) {
            ...
            if (strict) {
                std::cerr << "No value found for key: " << key << ". ";
                std::cerr << "Make sure all the settings files are loaded." << std::endl;
                exit(-1);
            } else {
                return -1;
            }
                          ...
                          int pos, bool useAsInitial)
{
  ...
  if(idx != -1)
  {
    ...
    /*std::cerr << "modify internal: key = " << key
         << ", value  = " << value
  ...
  else
  {
    ...
    /*std::cerr << "insert internal: key = " << key
         << ", value  = " << value
                          ...
                          int pos, bool useAsInitial)
{
  ...
  if(idx != -1)
  {
    ...
    /*std::cerr << "modify external: key = " << key
         << ", value = " << value
         << " @ index = " << idx
         << std::endl;*/
  }
  else
  {
    ...
    /*std::cerr << "insert external: key = " << key
         << ", value = " << value
         << " @ index = " << idx
         << std::endl;*/
  }
